...
class MerchantProductsPage extends StatefulWidget {
  ...
  const MerchantProductsPage({super.key, required this.merchantId});
...
class _MerchantProductsPageState extends State<MerchantProductsPage> {
  ...
  final List<_CategorySection> _sections = [
    _CategorySection(
      name: 'Plato fuerte',
      categoryId: '54912ce6-db1b-4178-8fc2-d8ce0323f199',
    ),
    _CategorySection(
      name: 'Postres',
      categoryId: '2ff6dcda-6625-44ac-bbc5-9beb6f066803',
    ),
    _CategorySection(
      name: 'Comida rápida',
      categoryId: 'f549b092-e486-4b40-ab12-891fd3321f77',
    ),
    _CategorySection(
      name: 'Bebidas',
      categoryId: 'c1812c32-c9f7-4ee4-badf-a0c435cc0760',
    ),
  ];
          ...
          return ListView(
            ...
            children: [
              for (var section in _sections) ...[
                ...
                if (products.any(
                  (p) => p.category_id == section.categoryId,
                )) ...[
                  Text(
                    section.name,
                      ...
                      return _ProductRow(
                        ...
                        onEdit: () {
                          Navigator.pushNamed(
                            ...
                            Routes.editProduct,
                            arguments:
                                p, // 👈 p es el producto actual que se está editando
                          ).then((_) {
                            // Recargar productos después de editar
                            setState(() {
                              final supa = Supabase.instance.client;
                              final ds = ProductsDataSourceImpl(
                                supabaseClient: supa,
                              );
                              final uc = FetchMerchantProductsUseCase(ds);
                              _productsFuture = uc.call(widget.merchantId);
                          ...
                          showDialog(
                            ...
                            builder:
                                (_) => AlertDialog(
                                  ...
                                  content: Text(
                                    '¿Estás seguro de que deseas eliminar "${p.name}"?',
                                  ),
                                  actions: [
                                    TextButton(
                                    ...
                                    ElevatedButton(
                                      ...
                                      onPressed: () async {
                                        ...
                                        await ds.deleteProduct(
                                          p.product_id,
                                        ); // 👈 Método que debes tener implementado

                                        // Recargar productos
                                        final uc = FetchMerchantProductsUseCase(
                                          ds,
                                        );
                                        setState(() {
                                          _productsFuture = uc.call(
                                            widget.merchantId,
                                          );
                                        });
